...
class DataTransferWithDate extends Command
{

    protected $signature = 'sync:scout-labs-data
    ...
    public function handle(): int
    {
        ...
        $startDate = $endDate->copy()->startOfYear()->addMonths(4)->next(CarbonInterface::SUNDAY);
        ...
        try {
            ...
            DB::connection('weather')
                ...
                ->orderBy('id')
                ->chunkById(500, function ($trapLocations) use ($regionId) {
            ...
            foreach($locations as $location) {
                ...
                // find the last active trap date
                $active = DB::table('contour_trap_counts')
                    ->where('location_id', $trap->id)
                    ->orderBy('survey_date', 'desc')
                    ->first();

                if($active != null) {
                    $newestRecordDate = Carbon::parse($active->survey_date);;
                }
                else
                {
                    $newestRecordDate = $startDate;
                }
                // TODO: date is always found as 2026-04 (which hasn't happened yet)
                $records = DB::connection('weather')->table('scout_labs_records')
                    ->where('trap_id', $trap->id)
                    ->where('pest_name', $pestName)
                    ->whereBetween('recorded_at', [$newestRecordDate, $endDate])
                    ->get();

                // TODO: Currently records is always null
            ...
            foreach($filteredRecords as $trapRecord) {
                DB::table('contour_trap_counts')->UpdateOrInsert([
                    ...
                    'model_id' => $pestID,
                    'survey_date' => Carbon::parse($trapRecord->recorded_at),
