...
def run_r_models(hourly_df):
    # Calculate chilling in Python
    temp = hourly_df['Temp'].values
    CU = chilling_units(temp)
    Utah = modified_utah_model(temp)
    NC = north_carolina_model(temp)
    DP = dynamic_model(temp)
    ...
    # R code below
    ro.r("""
        if (!require(fruclimadapt)) {
            install.packages("fruclimadapt", dependencies = TRUE, repos = "https://cloud.r-project.org", quiet = TRUE, Ncpus = 1)
        }
...
def stack_hourly(df, latitude):
    ...
    ro.r("""
        ...
        library(chillR)
        result <- chillR:::stack_hourly_temps(df_input, latitude = lat)[[1]]
    """)
...
# helper functions for chilling models
def chilling_units(temp):
    # Simple chill hours: 1 if temp < 7.2, 0 otherwise
    return np.where(temp < 7.2, 1, 0)

def modified_utah_model(temp):
    chill = np.zeros_like(temp, dtype=float)
    chill[(temp > 1.4) & (temp <= 2.4)] = 0.5
    chill[(temp > 2.4) & (temp <= 9.1)] = 1
    chill[(temp > 9.1) & (temp <= 12.4)] = 0.5
    chill[(temp > 15.9) & (temp <= 18.0)] = -0.5
    chill[temp > 18] = -1
    return chill

def north_carolina_model(temp):
    chill = np.zeros_like(temp, dtype=float)
    chill[(temp > 1.6) & (temp <= 7.2)] = 1
    chill[(temp > 7.2) & (temp <= 13.0)] = 0.5
    chill[(temp > 16.0) & (temp <= 19.0)] = -0.5
    chill[temp > 19] = -1
    return chill

def dynamic_model(temp):
    # Simplified dynamic model: similar to Utah but with accumulation
    # For simplicity, use modified Utah
    return modified_utah_model(temp)
